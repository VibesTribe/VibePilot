...
class MaintenanceAgent:
    ...
    def _poll_and_execute(self):
        ...
        # Claim next pending command atomically
        try:
            result = db.rpc(
                "claim_next_command",
    ...
    def _execute_create_branch(self, payload: Dict) -> Dict:
        ...
        try:
            ...
            if result.returncode != 0:
                return {"success": False, "error": f"Failed to checkout {base_branch}: {result.stderr}"}
            ...
            if result.returncode != 0:
                return {"success": False, "error": f"Failed to push branch: {result.stderr}"}
        ...
        try:
            ...
            if target not in result.stdout:
                if create_target:
                    ...
                    return {"success": False, "error": f"Target branch {target} does not exist"}
            
            # Checkout target
            result = self._git_command(["checkout", target])
            if result.returncode != 0:
                return {"success": False, "error": f"Failed to checkout {target}: {result.stderr}"}
            ...
            # Merge source
            result = self._git_command(["merge", "--no-ff", source, "-m", f"Merge {source} into {target}"])
            if result.returncode != 0:
    ...
    def _execute_delete_branch(self, payload: Dict) -> Dict:
        ...
        try:
            ...
            if result.returncode != 0:
                return {"success": False, "error": f"Failed to delete remote branch: {result.stderr}"}
