-- Migration 122: Complete pipeline alignment (with testing fix)
-- Rerunnable: auto-drops conflicting signatures before recreating.
--
-- BUGS FIXED:
-- 1. claim_for_review: matched wrong status → now accepts both review AND testing
-- 2. unlock_dependent_tasks: wrong column (depends_on→dependencies) and status (blocked→pending)
-- 3. claim_task: no dependency guard
-- 4. transition_task: no attempts increment + no auto-unlock on completion
...
-- 1. claim_for_review: accept both review AND testing status
...
-- 3. claim_task with dependency guard
...
-- 4. transition_task with attempts + auto-unlock
...
BEGIN
  ...
  SET status = p_new_status,
      ...
      completed_at = CASE WHEN p_new_status IN ('complete', 'merged', 'cancelled') THEN NOW() ELSE completed_at END,
      updated_at = NOW()
  WHERE id = p_task_id;
  GET DIAGNOSTICS v_updated = ROW_COUNT;
