-- Slice-based task numbering
-- Each slice (module) has its own task sequence: T001, T002, T003...
-- Prevents collisions when multiple plans create tasks in the same slice
...
-- Add comment
COMMENT ON FUNCTION get_next_task_number_for_slice(TEXT) IS 'Get the next sequential task number for a slice (module). Returns T001, T002, etc. Each slice has its own sequence.';
