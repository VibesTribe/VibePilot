...
|-----------|--------|-----|
| Logs | Not built | No log viewer component |
| Docs | Not built | No doc browser component |
| Admin panel | Not built | Feature branch exists, not merged |
| Review queue | Code exists but no live data | File-based, not Supabase-fed |
| Failures panel | Empty array stub | No Supabase query feeds it |
| Merge candidates | Empty array stub | No Supabase query feeds it |

### Known Issues:
| Issue | Details |
|-------|---------|
| **failed/escalated status invisible** | Both mapped to "pending" in adapter -- distinction lost |
| **Per-task costUsd hardcoded 0** | ROI shows aggregate but not per-task |
| **Review system file-based** | Uses static JSON, not Supabase -- won't show live pipeline data |
| **Local dev has no .env.local** | Local vibeflow checkout can't connect to Supabase (env only in Vercel) |
