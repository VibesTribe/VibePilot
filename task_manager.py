...
class TaskManager:
    def __init__(self, model_id: str = "deepseek-chat", courier: str = "opencode", platform: str = "deepseek-api"):
    ...
    def check_model_limits(self, model_id: str = None) -> Dict:
        ...
        if model["status"] != "active":
            return {"ok": False, "reason": model.get("status_reason", "Model not active")}
        
        req_pct = (model.get("request_used", 0) / model.get("request_limit", 1)) * 100 if model.get("request_limit") else 0
        tok_pct = (model.get("token_used", 0) / model.get("token_limit", 1)) * 100 if model.get("token_limit") else 0
        
        if req_pct >= 80 or tok_pct >= 80:
            return {"ok": False, "reason": f"Approaching limit (req:{req_pct:.0f}%, tok:{tok_pct:.0f}%)"}
        
        return {"ok": True, "request_used": req_pct, "token_used": tok_pct}
    
    def claim_next_task(self) -> Optional[Dict]:
        try:
            result = db.rpc("claim_next_task", {
    ...
    def create_task(self, title: str, task_type: str, priority: int = 5, dependencies: List[str] = None) -> str:
    ...
    def create_task_packet(self, task_id: str, prompt: str, tech_spec: Dict = None, expected_output: str = None, context: Dict = None) -> str:
        existing = db.table("task_packets").select("version").eq("task_id", task_id).order("version", desc=True).limit(1).execute()
        next_version = (existing.data[0]["version"] + 1) if existing.data else 1
        
        res = db.table("task_packets").insert({
            "task_id": task_id,
            "prompt": prompt,
    ...
    def complete_run(self, run_id: str, success: bool, result: Dict = None, tokens_used: int = 0, error: str = None):
    ...
    def update_task_status(self, task_id: str, status: str, result: Dict = None, failure_notes: str = None):
    ...
    def add_review(self, task_id: str, passed: bool, notes: str, reviewer: str = "system"):
        task = self.get_task(task_id)
    ...
    def add_test_results(self, task_id: str, passed: bool, results: List[Dict], coverage: int = None):
        task = self.get_task(task_id)
        tests = {
            "passed": passed,
            "results": results,
    ...
    def approve_and_merge(self, task_id: str, merged_by: str = "maintenance", branch_name: str = None):
        approval = {
            "passed": True,
            "merged_by": merged_by,
    ...
    def _check_dependent_tasks(self, completed_task_id: str):
        try:
            result = db.rpc("make_task_available", {"p_task_id": completed_task_id}).execute()
...
if __name__ == "__main__":
    ...
    for m in tm.get_active_models():
        print(f"  {m['id']}: {m['status']} (requests: {m.get('request_used', 0)}/{m.get('request_limit', '?')})")
