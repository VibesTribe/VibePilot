# VibePilot TODO - 2026-04-14

## Critical (do first)

### 1. Merge research-update-april2026 into main
The development branch is 26 commits ahead. All the knowledge layer, doc alignment,
pipeline YAML, gitree work is sitting on a branch. GitHub main is stale.
- Merge `research-update-april2026` into `main`
- Sync `~/vibepilot/` (running copy) with the new main
- Delete dead `research-considerations` branch (25 old research commits, 1286 behind)
- Going forward: work on main unless there's a reason not to

### 2. Rebuild the governor binary
The running binary (Apr 11) was compiled from the OLD main source. The Go source in
`~/VibePilot/` has newer code (gitree improvements, DAG engine, etc.) that isn't compiled.
- After merge: `cd ~/vibepilot/governor && go build -o governor ./cmd/governor/`
- Restart: `systemctl --user restart vibepilot-governor`

### 3. Get free API keys (user action)
...
### 4. Build model cascade into config
...
### 5. Wire cascade into governor routing
...
### 6. Update WYNTK (VIBEPILOT_WHAT_YOU_NEED_TO_KNOW.md)
...
### 9. Daily landscape research cron
...
- [x] JourneyKits landscape analysis (95 kits)
