...
class AgentComm:
    ...
    def send(
        self, to_agent: str, content: Dict[str, Any], message_type: str = "chat"
    ) -> Optional[str]:
        ...
        try:
            ...
            if result.data:
                ...
                print(
                    f"[AgentComm] Sent {message_type} to {to_agent}: {content.get('text', '')[:50]}..."
                )
                return msg_id
    ...
    def get_unread(self) -> list:
        ...
        client = self._get_service_client()
        if not client:
            print(f"[AgentComm] No client available for {self.agent_name}")
            return []
    ...
    def check_messages(self) -> list:
        """Robust message check - always works. Use this instead of get_unread()."""
        client = self._get_service_client()
        if not client:
            return []
        ...
        try:
            ...
            print(f"[AgentComm] check_messages error: {e}")
            return []
...
def send(
    to_agent: str, content: Dict[str, Any], message_type: str = "chat"
) -> Optional[str]:
