-- Migration 135: Upgrade kb_context_pack with semantic search via seed embeddings
-- Strategy: use keyword matching to find a seed symbol, then use its embedding
...
BEGIN
    ...
    -- Find a seed embedding from any symbol matching the query keywords
    ...
    -- 1. Symbols: semantic search via seed embedding + keyword fallback
    IF v_seed_embedding IS NOT NULL THEN
        ...
        FROM (
            SELECT * FROM (
                -- Semantic matches: find symbols similar to seed (capped at p_limit)
                SELECT s.name, s.qualified_name, s.kind, s.summary, s.file_id, s.line_start,
                    round((1 - (s.embedding <=> v_seed_embedding))::numeric, 2) * 5 as score
                FROM kb_code_symbols s
                WHERE s.embedding IS NOT NULL
                  AND (p_repo_id IS NULL OR s.repo_id = p_repo_id)
                  AND 1 - (s.embedding <=> v_seed_embedding) > 0.45
                  AND s.kind IN ('function', 'method', 'type', 'interface', 'struct', 'constant')
                ORDER BY s.embedding <=> v_seed_embedding
                LIMIT p_limit
            ) sem

            UNION ALL

            -- Keyword matches as fallback
            SELECT s.name, s.qualified_name, s.kind, s.summary, s.file_id, s.line_start,
                CASE
    ...
    ELSE
        -- No seed embedding found -- pure keyword matching
        RETURN QUERY
        SELECT 'symbols' as section,
            jsonb_agg(row_to_json(sym)::jsonb ORDER BY sym.score DESC) as content
        FROM (
    ...
    -- 2. Data flow: keyword split matching
    RETURN QUERY
    ...
    -- 3. Related docs: keyword split matching
    RETURN QUERY
    ...
    -- 4. Related decisions: keyword split matching
    RETURN QUERY
    ...
    -- 5. Knowledge items (rules, configs, patterns)
    RETURN QUERY
    ...
    -- 6. Repo map snippet: find files containing matching symbols
    RETURN QUERY
    ...
    -- 7. Topic-specific rules (keyword split)
    RETURN QUERY
    ...
    -- 8. Non-negotiable rules (ALWAYS included)
    RETURN QUERY
    ...
    -- 9. Project principles (ALWAYS included)
    RETURN QUERY
    ...
    -- 10. System overview (ALWAYS included -- built from VERIFIED sources only)
    v_pipeline := (
