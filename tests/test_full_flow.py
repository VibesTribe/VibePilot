...
class TestVibePilotFullFlow:
    ...
    def test_03_maintenance_can_claim_and_execute(self, maintenance):
        ...
        # Insert a test command directly
        command_id = self._insert_test_command("create_branch", {
            "branch_name": "task/TTEST-claim-test",
            "base_branch": "main"
        })
        
        print(f"Inserted test command: {command_id[:8]}")
        
        # Maintenance should claim it
        result = db.rpc("claim_next_command", {"p_agent_id": "test"}).execute()
        
        if result.data:
            claimed = result.data[0]
            assert claimed["command_id"] == command_id
            assert claimed["status"] == "in_progress"
            print(f"✅ Claimed command: {claimed['command_type']}")
            
            # Complete it
            db.rpc("complete_command", {
    ...
    def test_08_idempotency_prevents_duplicates(self, supervisor):
        ...
        result1 = supervisor.command_create_branch(
            task_id=task_id,
            branch_name="task/TTEST-idempotent"
        )
        ...
        command = db.table("maintenance_commands").select("*").eq("id", result1["command_id"]).execute()
        if command.data:
    ...
    def test_99_cleanup_test_data(self):
        ...
        result = db.table("maintenance_commands").delete().like("payload->>branch_name", "%TTEST%").execute()
    ...
    def _delete_test_command(self, command_id: str):
        ...
        try:
            db.table("maintenance_commands").delete().eq("id", command_id).execute()
        except:
            pass
...
def run_integration_tests():
    ...
    tests = [
        ...
        ("Queue create_branch", lambda: test_class.test_02_supervisor_can_queue_create_branch(SupervisorAgent())),
        ("Claim command", lambda: test_class.test_03_maintenance_can_claim_and_execute(MaintenanceAgent())),
        ("Status tracking", lambda: test_class.test_04_command_status_tracking(SupervisorAgent())),
        ("Rate limit status", lambda: test_class.test_05_rate_limit_status_format(ConcurrentOrchestrator())),
        ("Council routing", lambda: test_class.test_06_council_routing_structure(ConcurrentOrchestrator())),
        ("Protected branches", lambda: test_class.test_07_protected_branch_prevention(SupervisorAgent())),
        ("Idempotency", lambda: test_class.test_08_idempotency_prevents_duplicates(SupervisorAgent())),
    ]
