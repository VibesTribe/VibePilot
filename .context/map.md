# VibePilot Code Map
# Generated: 2026-05-07T20:54:17Z | Commit: 18965e86
# Auto-generated. Run build.sh to regenerate.

## governor/cmd/cleanup/main.go
main.go [68L]
  deps: time, github.com/vibepilot/governor/internal/db, syscall, log, os, fmt, os/signal, context
  API:
    fn main()
...
content_fetcher.go [48L]
  deps: net/http, os, path/filepath, fmt, log, io, context
  API:
...
handlers_council.go [522L]
  deps: time, encoding/json, log, fmt, github.com/vibepilot/governor/internal/db, sync, context, errors, github.com/vibepilot/governor/internal/gitree, github.com/vibepilot/governor/internal/runtime, os
...
handlers_maint.go [664L]
  deps: context, github.com/vibepilot/governor/internal/db, encoding/json, fmt, log, strings, time, os/exec, github.com/vibepilot/governor/internal/gitree, github.com/vibepilot/governor/internal/runtime
...
handlers_plan.go [745L]
  deps: context, github.com/vibepilot/governor/internal/gitree, log, time, github.com/vibepilot/governor/internal/runtime, encoding/json, strings, fmt, os, path/filepath, github.com/vibepilot/governor/internal/db
  API:
...
handlers_research.go [463L]
  deps: context, time, log, github.com/vibepilot/governor/internal/runtime, sync, github.com/vibepilot/governor/internal/db, fmt, encoding/json
...
handlers_task.go [1949L]
  deps: github.com/vibepilot/governor/internal/core, github.com/vibepilot/governor/internal/gitree, github.com/vibepilot/governor/internal/security, time, github.com/vibepilot/governor/internal/db, github.com/vibepilot/governor/internal/runtime, strings, encoding/json, github.com/vibepilot/governor/internal/connectors, context, fmt, os, log, path/filepath
...
handlers_testing.go [1451L]
  deps: github.com/vibepilot/governor/internal/runtime, log, os/exec, bytes, os, encoding/json, strings, time, context, github.com/vibepilot/governor/internal/db, path/filepath, fmt, github.com/vibepilot/governor/internal/gitree
...
helpers.go [213L]
  deps: strings, log, encoding/json, context, github.com/vibepilot/governor/internal/db
  API:
...
helpers_record.go [48L]
  deps: github.com/vibepilot/governor/internal/db, context, github.com/vibepilot/governor/internal/runtime, encoding/json, fmt
  API:
...
main.go [736L]
  deps: github.com/vibepilot/governor/internal/vault, path/filepath, github.com/vibepilot/governor/internal/dag, github.com/vibepilot/governor/internal/runtime, github.com/vibepilot/governor/internal/connectors, github.com/vibepilot/governor/internal/security, github.com/vibepilot/governor/internal/tools, syscall, time, github.com/vibepilot/governor/internal/core, fmt, github.com/vibepilot/governor/internal/memory, github.com/vibepilot/governor/internal/pgnotify, github.com/vibepilot/governor/internal/webhooks, github.com/vibepilot/governor/internal/kb, encoding/json, github.com/vibepilot/governor/internal/db, github.com/vibepilot/governor/internal/gitree, context, log, os/signal, github.com/vibepilot/governor/internal/mcp, os
  API:
    fn main()
...
recovery.go [443L]
  deps: github.com/vibepilot/governor/internal/runtime, time, github.com/vibepilot/governor/internal/db, log, context, github.com/vibepilot/governor/internal/core, encoding/json, fmt
  API:
...
smoke.go [252L]
  deps: io, bytes, net/http, encoding/json, fmt, time, log, os
  API:
...
startup_validate.go [503L]
  deps: os/exec, strings, context, fmt, time, encoding/json, log, os, path/filepath
  API:
...
validation.go [474L]
  deps: github.com/vibepilot/governor/internal/runtime, regexp, github.com/vibepilot/governor/internal/db, encoding/json, fmt, strings, github.com/vibepilot/governor/internal/gitree, log, strconv, context
  API:
    cl ⊛ TaskData
    cl ⊛ ValidationError
      fn ⊛ Error() → string
...
main.go [200L]
  deps: encoding/json, crypto/rand, fmt, crypto/cipher, log, os, crypto/sha256, bytes, io, net/http, crypto/aes, golang.org/x/crypto/pbkdf2, encoding/base64
  API:
    fn main()
    cl ⊛ Secret
...
courier.go [267L]
  deps: net/http, sync, time, fmt, io, context, encoding/json, bytes
...
runners.go [513L]
  deps: bufio, encoding/json, github.com/vibepilot/governor/internal/vault, net/http, context, io, fmt, strings, bytes, os/exec, time, github.com/vibepilot/governor/internal/runtime
...
test_runner.go [296L]
  deps: time, strings, fmt, os, context, encoding/json, path/filepath, os/exec
...
engine.go [233L]
  deps: log, context, strings, sync, time, fmt
  exports: NewEngine
  API:
    cl ⊛ NodeOutput
    if ⊛ NodeExecutor
...
postgres.go [685L]
  deps: github.com/jackc/pgx/v5/pgxpool, github.com/jackc/pgx/v5/pgtype, github.com/jackc/pgx/v5, context, strconv, encoding/binary, strings, encoding/json, fmt, time
  exports: NewPostgres
  API:
    cl ⊛ PostgresDB
...
supabase.go [285L]
  deps: context, encoding/json, net/url, regexp, strings, bytes, io, time, net/http, fmt
...
gitree.go [976L]
  deps: path/filepath, fmt, strings, log, bytes, regexp, context, os/exec, time, encoding/json, os
  exports: New
  API:
...
managed_repo.go [310L]
  deps: fmt, os/exec, context, strings, path/filepath, time, os, log
...
worktree.go [416L]
  deps: context, log, path/filepath, os, regexp, strings, fmt, time
...
kb.go [344L]
  deps: strings, github.com/jackc/pgx/v5/pgxpool, fmt, context
...
maintenance.go [346L]
  deps: github.com/vibepilot/governor/internal/db, github.com/vibepilot/governor/internal/gitree, github.com/vibepilot/governor/pkg/types, os, log, fmt, context, path/filepath, strings
  exports: New
  API:
    ty ⊛ RiskLevel
    ty ⊛ ChangeType
...
sandbox.go [165L]
  deps: time, log, os, fmt, path/filepath, os/exec, context, io
  API:
...
validation.go [248L]
  deps: encoding/json, os, path/filepath, strings, time, log, fmt
  API:
...
governor_server.go [211L]
  deps: github.com/mark3labs/mcp-go/mcp, github.com/mark3labs/mcp-go/server, fmt, os, log, strconv, context, github.com/vibepilot/governor/internal/runtime, encoding/json
...
registry.go [253L]
  deps: encoding/json, log, github.com/mark3labs/mcp-go/client, github.com/mark3labs/mcp-go/mcp, github.com/vibepilot/governor/internal/runtime, context, github.com/mark3labs/mcp-go/client/transport, sync, time, fmt
  exports: NewRegistry
  API:
    cl ⊛ ToolBinding
    cl ⊛ Registry
...
compactor.go [248L]
  deps: fmt, context, encoding/json, strings, github.com/vibepilot/governor/internal/db, github.com/vibepilot/governor/internal/runtime, time
...
listener.go [246L]
  deps: time, github.com/vibepilot/governor/internal/runtime, fmt, context, log, github.com/jackc/pgx/v5, encoding/json
  exports: NewListener
  API:
    if ⊛ SSEBroadcaster
    cl ⊛ Listener
...
config.go [1401L]
  deps: log, path/filepath, sync, context, encoding/json, fmt, os
...
context_builder.go [609L]
  deps: path/filepath, sync, strings, os, fmt, encoding/json, time, context
...
model_loader.go [364L]
  deps: time, context, github.com/vibepilot/governor/internal/db, os, fmt, path/filepath, encoding/json, log
...
model_scanner.go [519L]
  deps: log, strconv, context, sync, encoding/json, fmt, io, net/http, strings, time, github.com/vibepilot/governor/internal/db
...
parallel.go [230L]
  deps: context, log, fmt, sync/atomic, sync
...
research_action.go [561L]
  deps: fmt, context, sync, encoding/json, path/filepath, github.com/vibepilot/governor/internal/db, os, log
...
router.go [808L]
  deps: github.com/vibepilot/governor/internal/db, context, sync/atomic, encoding/json, log, sort
...
db_tools.go [238L]
  deps: fmt, encoding/json, context, regexp, github.com/vibepilot/governor/internal/db
...
file_tools.go [177L]
  deps: encoding/json, os, strings, fmt, path/filepath, context
...
registry.go [94L]
  deps: github.com/vibepilot/governor/internal/db, net/http, github.com/vibepilot/governor/internal/runtime, github.com/vibepilot/governor/internal/vault, github.com/vibepilot/governor/internal/gitree, time
  exports: RegisterAll
  API:
    cl ⊛ Dependencies
    fn init()
...
sandbox_tools.go [245L]
  deps: bytes, os/exec, strings, encoding/json, fmt, time, context, os, path/filepath
...
web_tools.go [231L]
  deps: encoding/json, io, net/url, net/http, strings, github.com/vibepilot/governor/internal/runtime, context, fmt
...
vault.go [506L]
  deps: encoding/json, crypto/aes, crypto/cipher, log, sync, golang.org/x/crypto/pbkdf2, fmt, context, github.com/vibepilot/governor/internal/db, io, time, os, crypto/rand, crypto/sha256, encoding/base64
...
server.go [1671L]
  deps: encoding/hex, path/filepath, context, strings, time, fmt, encoding/json, os, net/http, crypto/sha256, crypto/hmac, github.com/vibepilot/governor/internal/runtime, log, io
