# VibePilot Code Map
# Generated: 2026-05-08T02:18:32Z | Commit: 478473d0
# Auto-generated. Run build.sh to regenerate.

## governor/cmd/cleanup/main.go
main.go [68L]
  deps: context, log, time, os, syscall, os/signal, github.com/vibepilot/governor/internal/db, fmt
  API:
    fn main()
...
content_fetcher.go [48L]
  deps: io, net/http, fmt, context, os, log, path/filepath
  API:
...
handlers_council.go [522L]
  deps: sync, github.com/vibepilot/governor/internal/gitree, log, github.com/vibepilot/governor/internal/runtime, github.com/vibepilot/governor/internal/db, os, context, fmt, encoding/json, errors, time
...
handlers_maint.go [664L]
  deps: encoding/json, log, strings, time, context, github.com/vibepilot/governor/internal/db, fmt, github.com/vibepilot/governor/internal/gitree, github.com/vibepilot/governor/internal/runtime, os/exec
...
handlers_plan.go [745L]
  deps: github.com/vibepilot/governor/internal/db, encoding/json, fmt, context, path/filepath, time, strings, log, github.com/vibepilot/governor/internal/gitree, os, github.com/vibepilot/governor/internal/runtime
  API:
...
handlers_research.go [463L]
  deps: sync, time, context, encoding/json, github.com/vibepilot/governor/internal/runtime, fmt, log, github.com/vibepilot/governor/internal/db
...
handlers_task.go [1949L]
  deps: fmt, github.com/vibepilot/governor/internal/security, github.com/vibepilot/governor/internal/db, path/filepath, time, encoding/json, os, github.com/vibepilot/governor/internal/gitree, github.com/vibepilot/governor/internal/core, log, strings, context, github.com/vibepilot/governor/internal/connectors, github.com/vibepilot/governor/internal/runtime
...
handlers_testing.go [1451L]
  deps: strings, bytes, os, path/filepath, github.com/vibepilot/governor/internal/db, time, github.com/vibepilot/governor/internal/gitree, fmt, github.com/vibepilot/governor/internal/runtime, encoding/json, log, context, os/exec
...
main.go [736L]
  deps: github.com/vibepilot/governor/internal/vault, github.com/vibepilot/governor/internal/dag, path/filepath, github.com/vibepilot/governor/internal/pgnotify, github.com/vibepilot/governor/internal/db, github.com/vibepilot/governor/internal/kb, context, github.com/vibepilot/governor/internal/gitree, github.com/vibepilot/governor/internal/mcp, github.com/vibepilot/governor/internal/tools, time, syscall, github.com/vibepilot/governor/internal/runtime, log, encoding/json, os, github.com/vibepilot/governor/internal/connectors, os/signal, github.com/vibepilot/governor/internal/security, github.com/vibepilot/governor/internal/webhooks, github.com/vibepilot/governor/internal/core, github.com/vibepilot/governor/internal/memory, fmt
  API:
    fn main()
...
recovery.go [443L]
  deps: context, github.com/vibepilot/governor/internal/core, github.com/vibepilot/governor/internal/db, time, log, encoding/json, github.com/vibepilot/governor/internal/runtime, fmt
  API:
...
smoke.go [252L]
  deps: io, net/http, encoding/json, fmt, log, os, bytes, time
  API:
...
startup_validate.go [503L]
  deps: context, os/exec, fmt, log, path/filepath, os, time, strings, encoding/json
  API:
...
validation.go [474L]
  deps: context, github.com/vibepilot/governor/internal/gitree, github.com/vibepilot/governor/internal/runtime, log, strconv, strings, fmt, regexp, github.com/vibepilot/governor/internal/db, encoding/json
  API:
    cl ⊛ TaskData
    cl ⊛ ValidationError
      fn ⊛ Error() → string
...
main.go [200L]
  deps: crypto/sha256, net/http, crypto/cipher, os, fmt, io, golang.org/x/crypto/pbkdf2, encoding/base64, log, bytes, encoding/json, crypto/aes, crypto/rand
  API:
    fn main()
    cl ⊛ Secret
...
courier.go [267L]
  deps: fmt, encoding/json, sync, net/http, bytes, io, context, time
...
runners.go [513L]
  deps: encoding/json, bytes, github.com/vibepilot/governor/internal/runtime, context, fmt, io, bufio, os/exec, strings, net/http, time, github.com/vibepilot/governor/internal/vault
...
state.go [299L]
  deps: fmt, sync, time, encoding/json, context
...
test_runner.go [296L]
  deps: time, fmt, os/exec, os, path/filepath, strings, encoding/json, context
...
engine.go [233L]
  deps: sync, fmt, log, context, strings, time
  exports: NewEngine
  API:
    cl ⊛ NodeOutput
    if ⊛ NodeExecutor
...
postgres.go [685L]
  deps: github.com/jackc/pgx/v5, context, encoding/json, time, github.com/jackc/pgx/v5/pgxpool, strings, strconv, encoding/binary, fmt, github.com/jackc/pgx/v5/pgtype
  exports: NewPostgres
  API:
    cl ⊛ PostgresDB
...
supabase.go [285L]
  deps: net/url, fmt, bytes, net/http, context, encoding/json, strings, io, time, regexp
...
gitree.go [976L]
  deps: log, context, path/filepath, regexp, strings, os/exec, time, bytes, encoding/json, fmt, os
  exports: New
  API:
...
managed_repo.go [310L]
  deps: fmt, os/exec, context, os, log, strings, path/filepath, time
...
worktree.go [416L]
  deps: fmt, time, strings, regexp, log, path/filepath, os, context
...
maintenance.go [346L]
  deps: path/filepath, strings, context, log, os, github.com/vibepilot/governor/internal/gitree, github.com/vibepilot/governor/pkg/types, github.com/vibepilot/governor/internal/db, fmt
  exports: New
  API:
    ty ⊛ RiskLevel
    ty ⊛ ChangeType
...
sandbox.go [165L]
  deps: fmt, io, context, time, os, path/filepath, log, os/exec
  API:
...
validation.go [248L]
  deps: encoding/json, strings, log, fmt, time, os, path/filepath
  API:
...
governor_server.go [211L]
  deps: github.com/vibepilot/governor/internal/runtime, strconv, fmt, context, log, os, encoding/json, github.com/mark3labs/mcp-go/mcp, github.com/mark3labs/mcp-go/server
...
registry.go [253L]
  deps: context, encoding/json, time, github.com/mark3labs/mcp-go/client/transport, log, github.com/mark3labs/mcp-go/mcp, github.com/vibepilot/governor/internal/runtime, github.com/mark3labs/mcp-go/client, sync, fmt
  exports: NewRegistry
  API:
    cl ⊛ ToolBinding
    cl ⊛ Registry
...
compactor.go [248L]
  deps: encoding/json, fmt, github.com/vibepilot/governor/internal/runtime, time, context, strings, github.com/vibepilot/governor/internal/db
...
service.go [302L]
  deps: time, fmt, encoding/json, context, github.com/vibepilot/governor/internal/db
  exports: New
  API:
    cl ⊛ Config
    cl ⊛ Rule
...
listener.go [246L]
  deps: time, fmt, encoding/json, context, log, github.com/jackc/pgx/v5, github.com/vibepilot/governor/internal/runtime
  exports: NewListener
  API:
    if ⊛ SSEBroadcaster
    cl ⊛ Listener
...
config.go [1401L]
  deps: encoding/json, os, path/filepath, log, context, sync, fmt
...
connector_tracker.go [283L]
  deps: sync, encoding/json, time, log, context
...
context_builder.go [609L]
  deps: time, context, path/filepath, sync, encoding/json, fmt, strings, os
...
events.go [129L]
  deps: time, encoding/json, sync, log, context
...
model_loader.go [364L]
  deps: encoding/json, log, time, context, path/filepath, github.com/vibepilot/governor/internal/db, fmt, os
...
model_scanner.go [519L]
  deps: time, fmt, sync, log, encoding/json, context, github.com/vibepilot/governor/internal/db, strconv, io, strings, net/http
...
research_action.go [561L]
  deps: github.com/vibepilot/governor/internal/db, sync, os, log, encoding/json, path/filepath, fmt, context
...
session.go [286L]
  deps: time, encoding/json, context, fmt, strings
...
usage_tracker.go [915L]
  deps: context, sync, encoding/json, fmt, time, log
...
db_tools.go [238L]
  deps: github.com/vibepilot/governor/internal/db, context, fmt, encoding/json, regexp
...
registry.go [94L]
  deps: time, github.com/vibepilot/governor/internal/gitree, github.com/vibepilot/governor/internal/vault, github.com/vibepilot/governor/internal/db, net/http, github.com/vibepilot/governor/internal/runtime
  exports: RegisterAll
  API:
    cl ⊛ Dependencies
    fn init()
...
sandbox_tools.go [245L]
  deps: os, strings, os/exec, path/filepath, fmt, context, bytes, encoding/json, time
...
web_tools.go [231L]
  deps: net/http, fmt, encoding/json, io, net/url, strings, context, github.com/vibepilot/governor/internal/runtime
...
vault.go [506L]
  deps: golang.org/x/crypto/pbkdf2, context, fmt, github.com/vibepilot/governor/internal/db, crypto/rand, encoding/base64, log, time, encoding/json, crypto/cipher, crypto/aes, io, os, sync, crypto/sha256
...
server.go [1672L]
  deps: time, net/http, crypto/hmac, io, path/filepath, strings, github.com/vibepilot/governor/internal/runtime, crypto/sha256, fmt, encoding/json, encoding/hex, context, log, os
