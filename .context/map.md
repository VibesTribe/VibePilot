# VibePilot Code Map
# Generated: 2026-05-14T16:46:02Z | Commit: 242d5cf9
# Auto-generated. Run build.sh to regenerate.

## governor/cmd/cleanup/main.go
main.go [68L]
  deps: log, github.com/vibepilot/governor/internal/db, os/signal, context, syscall, os, fmt, time
  API:
    fn main()
...
content_fetcher.go [48L]
  deps: log, context, os, net/http, io, fmt, path/filepath
  API:
...
handlers_council.go [522L]
  deps: errors, encoding/json, os, context, fmt, github.com/vibepilot/governor/internal/runtime, sync, time, github.com/vibepilot/governor/internal/gitree, github.com/vibepilot/governor/internal/db, log
...
handlers_maint.go [664L]
  deps: os/exec, time, github.com/vibepilot/governor/internal/db, context, github.com/vibepilot/governor/internal/runtime, github.com/vibepilot/governor/internal/gitree, strings, encoding/json, log, fmt
...
handlers_plan.go [745L]
  deps: os, time, github.com/vibepilot/governor/internal/gitree, path/filepath, log, encoding/json, context, fmt, github.com/vibepilot/governor/internal/db, github.com/vibepilot/governor/internal/runtime, strings
  API:
...
handlers_research.go [463L]
  deps: log, time, github.com/vibepilot/governor/internal/db, context, github.com/vibepilot/governor/internal/runtime, encoding/json, fmt, sync
...
handlers_task.go [1955L]
  deps: fmt, time, os, github.com/vibepilot/governor/internal/security, log, strings, path/filepath, github.com/vibepilot/governor/internal/connectors, encoding/json, github.com/vibepilot/governor/internal/core, context, github.com/vibepilot/governor/internal/db, github.com/vibepilot/governor/internal/gitree, github.com/vibepilot/governor/internal/runtime
...
handlers_testing.go [1451L]
  deps: github.com/vibepilot/governor/internal/db, context, os/exec, encoding/json, strings, fmt, github.com/vibepilot/governor/internal/gitree, os, log, bytes, path/filepath, github.com/vibepilot/governor/internal/runtime, time
...
main.go [906L]
  deps: github.com/vibepilot/governor/internal/visualqa, github.com/vibepilot/governor/internal/designpreview, github.com/vibepilot/governor/internal/kb, github.com/vibepilot/governor/internal/webhooks, time, os/signal, encoding/json, fmt, github.com/vibepilot/governor/internal/mcp, github.com/vibepilot/governor/internal/gitree, os, github.com/vibepilot/governor/internal/memory, github.com/vibepilot/governor/internal/tools, github.com/vibepilot/governor/internal/db, github.com/vibepilot/governor/internal/security, path/filepath, github.com/vibepilot/governor/internal/dag, context, log, syscall, github.com/vibepilot/governor/internal/pgnotify, github.com/vibepilot/governor/internal/runtime, github.com/vibepilot/governor/internal/core, github.com/vibepilot/governor/internal/connectors, github.com/vibepilot/governor/internal/vault
  API:
    fn main()
...
recovery.go [443L]
  deps: fmt, github.com/vibepilot/governor/internal/db, time, context, log, encoding/json, github.com/vibepilot/governor/internal/core, github.com/vibepilot/governor/internal/runtime
  API:
...
smoke.go [252L]
  deps: bytes, time, os, encoding/json, log, net/http, fmt, io
  API:
...
startup_validate.go [508L]
  deps: context, fmt, os, os/exec, path/filepath, encoding/json, log, time, strings
  API:
...
validation.go [474L]
  deps: encoding/json, github.com/vibepilot/governor/internal/gitree, context, fmt, regexp, strconv, log, strings, github.com/vibepilot/governor/internal/db, github.com/vibepilot/governor/internal/runtime
  API:
    cl ⊛ TaskData
    cl ⊛ ValidationError
      fn ⊛ Error() → string
...
main.go [200L]
  deps: crypto/aes, encoding/json, fmt, io, bytes, log, crypto/sha256, crypto/rand, net/http, os, golang.org/x/crypto/pbkdf2, encoding/base64, crypto/cipher
  API:
    fn main()
    cl ⊛ Secret
...
courier.go [577L]
  deps: fmt, context, encoding/json, net/http, sync, strings, time, os/exec, io, os, bytes
...
runners.go [513L]
  deps: github.com/vibepilot/governor/internal/vault, context, fmt, io, os/exec, time, encoding/json, github.com/vibepilot/governor/internal/runtime, bufio, net/http, bytes, strings
...
test_runner.go [296L]
  deps: fmt, encoding/json, os, path/filepath, context, time, os/exec, strings
...
engine.go [233L]
  deps: fmt, context, strings, sync, time, log
  exports: NewEngine
  API:
    cl ⊛ NodeOutput
    if ⊛ NodeExecutor
...
postgres.go [685L]
  deps: strings, fmt, github.com/jackc/pgx/v5, time, github.com/jackc/pgx/v5/pgxpool, encoding/binary, strconv, context, github.com/jackc/pgx/v5/pgtype, encoding/json
  exports: NewPostgres
  API:
    cl ⊛ PostgresDB
...
supabase.go [285L]
  deps: time, net/http, context, regexp, encoding/json, io, strings, bytes, net/url, fmt
...
designpreview.go [161L]
  deps: time, encoding/json, fmt, context, github.com/google/uuid
...
generator.go [236L]
  deps: os/exec, bytes, encoding/base64, os, time, path/filepath, net/http, context, encoding/json, io, fmt
...
persist.go [94L]
  deps: encoding/json, time, github.com/google/uuid, fmt, context, path/filepath, os
...
gitree.go [976L]
  deps: bytes, encoding/json, context, fmt, os, time, regexp, strings, log, os/exec, path/filepath
  exports: New
  API:
...
managed_repo.go [310L]
  deps: path/filepath, strings, os/exec, context, fmt, log, time, os
...
worktree.go [416L]
  deps: strings, os, log, time, context, regexp, fmt, path/filepath
...
maintenance.go [346L]
  deps: fmt, log, context, path/filepath, github.com/vibepilot/governor/internal/gitree, github.com/vibepilot/governor/pkg/types, strings, github.com/vibepilot/governor/internal/db, os
  exports: New
  API:
    ty ⊛ RiskLevel
    ty ⊛ ChangeType
...
sandbox.go [165L]
  deps: path/filepath, os, os/exec, io, log, context, fmt, time
  API:
...
validation.go [248L]
  deps: path/filepath, os, encoding/json, strings, fmt, time, log
  API:
...
governor_server.go [211L]
  deps: os, log, strconv, context, github.com/mark3labs/mcp-go/server, github.com/mark3labs/mcp-go/mcp, encoding/json, github.com/vibepilot/governor/internal/runtime, fmt
...
registry.go [253L]
  deps: sync, github.com/mark3labs/mcp-go/client/transport, encoding/json, github.com/mark3labs/mcp-go/client, log, fmt, context, github.com/mark3labs/mcp-go/mcp, github.com/vibepilot/governor/internal/runtime, time
  exports: NewRegistry
  API:
    cl ⊛ ToolBinding
    cl ⊛ Registry
...
compactor.go [248L]
  deps: github.com/vibepilot/governor/internal/runtime, strings, context, fmt, encoding/json, time, github.com/vibepilot/governor/internal/db
...
listener.go [246L]
  deps: encoding/json, context, log, fmt, github.com/jackc/pgx/v5, time, github.com/vibepilot/governor/internal/runtime
  exports: NewListener
  API:
    if ⊛ SSEBroadcaster
    cl ⊛ Listener
...
config.go [1415L]
  deps: os, context, encoding/json, fmt, log, path/filepath, sync
...
context_builder.go [609L]
  deps: fmt, os, time, context, sync, encoding/json, path/filepath, strings
...
credit_poller.go [193L]
  deps: io, sync, context, encoding/json, fmt, log, net/http, strings, time
...
events.go [129L]
  deps: context, time, log, encoding/json, sync
...
model_loader.go [354L]
  deps: fmt, encoding/json, os, github.com/vibepilot/governor/internal/db, context, log, time, path/filepath
...
model_scanner.go [519L]
  deps: github.com/vibepilot/governor/internal/db, io, strconv, log, encoding/json, time, fmt, net/http, sync, strings, context
...
parallel.go [230L]
  deps: log, sync, fmt, context, sync/atomic
...
research_action.go [561L]
  deps: path/filepath, encoding/json, fmt, sync, log, github.com/vibepilot/governor/internal/db, context, os
...
router.go [808L]
  deps: context, sync/atomic, sort, encoding/json, github.com/vibepilot/governor/internal/db, log
...
usage_tracker.go [1004L]
  deps: sync, encoding/json, fmt, time, context, log
...
db_tools.go [238L]
  deps: encoding/json, fmt, regexp, context, github.com/vibepilot/governor/internal/db
...
registry.go [94L]
  deps: github.com/vibepilot/governor/internal/db, github.com/vibepilot/governor/internal/gitree, time, github.com/vibepilot/governor/internal/vault, github.com/vibepilot/governor/internal/runtime, net/http
  exports: RegisterAll
  API:
    cl ⊛ Dependencies
    fn init()
...
sandbox_tools.go [245L]
  deps: strings, path/filepath, context, time, encoding/json, bytes, fmt, os, os/exec
...
web_tools.go [231L]
  deps: strings, net/http, net/url, github.com/vibepilot/governor/internal/runtime, io, fmt, context, encoding/json
...
vault.go [506L]
  deps: fmt, crypto/aes, time, golang.org/x/crypto/pbkdf2, crypto/cipher, encoding/base64, crypto/rand, context, github.com/vibepilot/governor/internal/db, encoding/json, io, log, sync, crypto/sha256, os
...
audit.go [484L]
  deps: encoding/base64, os, strings, context, time, fmt, encoding/json
  API:
    cl ⊛ AuditResult
    cl ⊛ AuditIssue
...
baseline.go [164L]
  deps: path/filepath, context, encoding/json, bytes, fmt, os/exec, os, time
  API:
    cl ⊛ BaselineEntry
    ty ⊛ BaselineManifest
...
capture.go [513L]
  deps: encoding/json, path/filepath, os, os/exec, fmt, context, time, strconv
  API:
    cl ⊛ CaptureResult
    cl ⊛ UIIssue
    cl ⊛ UICaptureReport
...
compare.go [450L]
  deps: time, io, encoding/base64, fmt, encoding/json, strings, bytes, context, net/http, os
  API:
    cl ⊛ VisionProvider
...
db.go [78L]
  deps: context, os, database/sql, fmt, github.com/jackc/pgx/v5/pgxpool, github.com/jackc/pgx/v5
...
fixengine.go [437L]
  deps: strings, regexp, context, path/filepath, time, os, encoding/json, fmt, os/exec
...
visualqa.go [648L]
  deps: fmt, context, strings, encoding/json, github.com/google/uuid, regexp, time, github.com/jackc/pgx/v5
  exports: NewVisualQA
  API:
    cl ⊛ Config
...
github.go [282L]
  deps: context, encoding/json, github.com/vibepilot/governor/internal/db, strings, log, path/filepath
...
server.go [2158L]
  deps: os, io, fmt, crypto/hmac, github.com/vibepilot/governor/internal/runtime, path/filepath, strings, encoding/hex, log, crypto/sha256, encoding/json, strconv, context, net/http, os/exec, time
...
visualqa.go [216L]
  deps: net/http, encoding/json, log, time, context, fmt, io
  API:
    if ⊛ VisualQARunner
