# VibePilot Code Map
# Generated: 2026-05-15T04:45:50Z | Commit: ef5f5384
# Auto-generated. Run build.sh to regenerate.

## governor/cmd/cleanup/main.go
main.go [68L]
  deps: context, os/signal, log, time, github.com/vibepilot/governor/internal/db, os, fmt, syscall
  API:
    fn main()
...
content_fetcher.go [48L]
  deps: net/http, os, path/filepath, fmt, io, context, log
  API:
...
handlers_council.go [522L]
  deps: github.com/vibepilot/governor/internal/db, errors, fmt, log, time, github.com/vibepilot/governor/internal/gitree, github.com/vibepilot/governor/internal/runtime, context, encoding/json, os, sync
...
handlers_maint.go [664L]
  deps: os/exec, strings, time, github.com/vibepilot/governor/internal/gitree, github.com/vibepilot/governor/internal/runtime, fmt, encoding/json, context, log, github.com/vibepilot/governor/internal/db
...
handlers_plan.go [745L]
  deps: github.com/vibepilot/governor/internal/gitree, strings, encoding/json, github.com/vibepilot/governor/internal/db, github.com/vibepilot/governor/internal/runtime, fmt, path/filepath, context, time, log, os
  API:
...
handlers_research.go [463L]
  deps: context, log, time, fmt, github.com/vibepilot/governor/internal/runtime, encoding/json, github.com/vibepilot/governor/internal/db, sync
...
handlers_task.go [1955L]
  deps: github.com/vibepilot/governor/internal/runtime, github.com/vibepilot/governor/internal/security, github.com/vibepilot/governor/internal/connectors, encoding/json, context, log, strings, time, github.com/vibepilot/governor/internal/db, fmt, github.com/vibepilot/governor/internal/core, os, github.com/vibepilot/governor/internal/gitree, path/filepath
...
handlers_testing.go [1451L]
  deps: encoding/json, context, log, time, github.com/vibepilot/governor/internal/runtime, os/exec, path/filepath, strings, github.com/vibepilot/governor/internal/db, github.com/vibepilot/governor/internal/gitree, fmt, bytes, os
...
main.go [906L]
  deps: encoding/json, context, github.com/vibepilot/governor/internal/connectors, github.com/vibepilot/governor/internal/pgnotify, github.com/vibepilot/governor/internal/runtime, github.com/vibepilot/governor/internal/webhooks, github.com/vibepilot/governor/internal/gitree, time, os/signal, path/filepath, github.com/vibepilot/governor/internal/vault, github.com/vibepilot/governor/internal/core, github.com/vibepilot/governor/internal/dag, github.com/vibepilot/governor/internal/mcp, github.com/vibepilot/governor/internal/visualqa, log, github.com/vibepilot/governor/internal/security, github.com/vibepilot/governor/internal/tools, github.com/vibepilot/governor/internal/kb, github.com/vibepilot/governor/internal/memory, github.com/vibepilot/governor/internal/db, os, github.com/vibepilot/governor/internal/designpreview, syscall, fmt
  API:
    fn main()
...
recovery.go [443L]
  deps: github.com/vibepilot/governor/internal/runtime, encoding/json, fmt, github.com/vibepilot/governor/internal/core, github.com/vibepilot/governor/internal/db, log, time, context
  API:
...
smoke.go [252L]
  deps: fmt, os, time, net/http, log, io, bytes, encoding/json
  API:
...
startup_validate.go [508L]
  deps: log, os/exec, fmt, context, encoding/json, os, strings, path/filepath, time
  API:
...
validation.go [474L]
  deps: github.com/vibepilot/governor/internal/runtime, encoding/json, regexp, strings, context, github.com/vibepilot/governor/internal/db, strconv, fmt, log, github.com/vibepilot/governor/internal/gitree
  API:
    cl ⊛ TaskData
    cl ⊛ ValidationError
      fn ⊛ Error() → string
...
main.go [200L]
  deps: net/http, os, golang.org/x/crypto/pbkdf2, bytes, log, encoding/base64, crypto/rand, crypto/sha256, crypto/cipher, encoding/json, fmt, crypto/aes, io
  API:
    fn main()
    cl ⊛ Secret
...
courier.go [577L]
  deps: encoding/json, io, os, fmt, net/http, bytes, context, os/exec, strings, sync, time
...
runners.go [513L]
  deps: github.com/vibepilot/governor/internal/runtime, io, bufio, bytes, context, encoding/json, fmt, os/exec, strings, time, github.com/vibepilot/governor/internal/vault, net/http
...
state.go [299L]
  deps: encoding/json, time, sync, context, fmt
...
test_runner.go [296L]
  deps: path/filepath, context, os/exec, os, time, fmt, strings, encoding/json
...
engine.go [233L]
  deps: sync, strings, log, context, time, fmt
  exports: NewEngine
  API:
    cl ⊛ NodeOutput
    if ⊛ NodeExecutor
...
postgres.go [685L]
  deps: strconv, github.com/jackc/pgx/v5, github.com/jackc/pgx/v5/pgxpool, time, fmt, encoding/binary, encoding/json, context, strings, github.com/jackc/pgx/v5/pgtype
  exports: NewPostgres
  API:
    cl ⊛ PostgresDB
...
rpc.go [282L]
  deps: fmt, sync, encoding/json, context
...
supabase.go [285L]
  deps: strings, context, regexp, time, net/http, bytes, fmt, net/url, io, encoding/json
...
generator.go [236L]
  deps: fmt, io, net/http, bytes, encoding/json, context, encoding/base64, os, path/filepath, time, os/exec
...
persist.go [94L]
  deps: path/filepath, time, fmt, context, github.com/google/uuid, os, encoding/json
...
gitree.go [976L]
  deps: log, time, bytes, os/exec, os, strings, path/filepath, encoding/json, context, fmt, regexp
  exports: New
  API:
...
managed_repo.go [310L]
  deps: log, os, os/exec, strings, context, path/filepath, time, fmt
...
worktree.go [416L]
  deps: context, fmt, path/filepath, regexp, time, strings, os, log
...
maintenance.go [346L]
  deps: os, github.com/vibepilot/governor/internal/gitree, context, path/filepath, github.com/vibepilot/governor/internal/db, strings, log, fmt, github.com/vibepilot/governor/pkg/types
  exports: New
  API:
    ty ⊛ RiskLevel
    ty ⊛ ChangeType
...
sandbox.go [165L]
  deps: io, path/filepath, os/exec, fmt, context, log, os, time
  API:
...
validation.go [248L]
  deps: strings, time, log, encoding/json, fmt, os, path/filepath
  API:
...
governor_server.go [211L]
  deps: strconv, github.com/mark3labs/mcp-go/server, log, fmt, encoding/json, os, github.com/vibepilot/governor/internal/runtime, context, github.com/mark3labs/mcp-go/mcp
...
registry.go [253L]
  deps: fmt, github.com/mark3labs/mcp-go/mcp, github.com/vibepilot/governor/internal/runtime, log, encoding/json, context, github.com/mark3labs/mcp-go/client, github.com/mark3labs/mcp-go/client/transport, time, sync
  exports: NewRegistry
  API:
    cl ⊛ ToolBinding
    cl ⊛ Registry
...
compactor.go [248L]
  deps: github.com/vibepilot/governor/internal/runtime, fmt, strings, encoding/json, github.com/vibepilot/governor/internal/db, time, context
...
listener.go [246L]
  deps: encoding/json, log, fmt, context, github.com/jackc/pgx/v5, github.com/vibepilot/governor/internal/runtime, time
  exports: NewListener
  API:
    if ⊛ SSEBroadcaster
    cl ⊛ Listener
...
config.go [1415L]
  deps: path/filepath, encoding/json, os, fmt, log, context, sync
...
context_builder.go [609L]
  deps: context, time, fmt, strings, sync, path/filepath, encoding/json, os
...
credit_poller.go [193L]
  deps: io, net/http, strings, sync, context, fmt, time, log, encoding/json
...
model_loader.go [354L]
  deps: github.com/vibepilot/governor/internal/db, encoding/json, context, os, time, path/filepath, log, fmt
...
model_scanner.go [519L]
  deps: log, github.com/vibepilot/governor/internal/db, net/http, fmt, sync, io, context, time, strconv, encoding/json, strings
...
platform_tracker_v2.go [481L]
  deps: time, context, fmt, log, sync, encoding/json
...
research_action.go [561L]
  deps: fmt, encoding/json, log, path/filepath, github.com/vibepilot/governor/internal/db, os, context, sync
...
router.go [808L]
  deps: context, sync/atomic, encoding/json, log, github.com/vibepilot/governor/internal/db, sort
...
session.go [286L]
  deps: context, time, strings, fmt, encoding/json
...
usage_tracker.go [1004L]
  deps: fmt, log, context, sync, encoding/json, time
...
db_tools.go [238L]
  deps: regexp, github.com/vibepilot/governor/internal/db, encoding/json, context, fmt
...
file_tools.go [177L]
  deps: path/filepath, os, strings, encoding/json, context, fmt
...
registry.go [94L]
  deps: github.com/vibepilot/governor/internal/gitree, time, github.com/vibepilot/governor/internal/runtime, github.com/vibepilot/governor/internal/vault, github.com/vibepilot/governor/internal/db, net/http
  exports: RegisterAll
  API:
    cl ⊛ Dependencies
    fn init()
...
sandbox_tools.go [245L]
  deps: os, context, encoding/json, bytes, path/filepath, strings, time, fmt, os/exec
...
web_tools.go [231L]
  deps: io, strings, encoding/json, github.com/vibepilot/governor/internal/runtime, context, net/url, fmt, net/http
...
vault.go [506L]
  deps: crypto/aes, golang.org/x/crypto/pbkdf2, context, crypto/rand, encoding/json, crypto/sha256, io, time, encoding/base64, sync, fmt, os, log, github.com/vibepilot/governor/internal/db, crypto/cipher
...
audit.go [484L]
  deps: time, encoding/base64, fmt, encoding/json, context, os, strings
  API:
    cl ⊛ AuditResult
    cl ⊛ AuditIssue
...
baseline.go [164L]
  deps: os, path/filepath, context, fmt, encoding/json, bytes, os/exec, time
  API:
    cl ⊛ BaselineEntry
    ty ⊛ BaselineManifest
...
capture.go [513L]
  deps: encoding/json, path/filepath, os/exec, time, context, fmt, os, strconv
  API:
    cl ⊛ CaptureResult
    cl ⊛ UIIssue
    cl ⊛ UICaptureReport
...
compare.go [450L]
  deps: fmt, encoding/json, io, net/http, os, bytes, strings, time, encoding/base64, context
  API:
    cl ⊛ VisionProvider
...
db.go [78L]
  deps: github.com/jackc/pgx/v5/pgxpool, github.com/jackc/pgx/v5, database/sql, fmt, os, context
...
fixengine.go [437L]
  deps: strings, os/exec, fmt, context, regexp, path/filepath, time, encoding/json, os
...
visualqa.go [648L]
  deps: encoding/json, context, time, regexp, strings, fmt, github.com/jackc/pgx/v5, github.com/google/uuid
  exports: NewVisualQA
  API:
    cl ⊛ Config
...
designpreview.go [177L]
  deps: context, net/http, encoding/json, fmt, log
  API:
    if ⊛ DesignPreviewer
...
github.go [282L]
  deps: github.com/vibepilot/governor/internal/db, log, path/filepath, context, strings, encoding/json
...
server.go [2158L]
  deps: crypto/sha256, encoding/hex, io, strings, encoding/json, time, fmt, github.com/vibepilot/governor/internal/runtime, crypto/hmac, strconv, context, log, net/http, path/filepath, os/exec, os
...
visualqa.go [216L]
  deps: net/http, time, fmt, encoding/json, context, log, io
  API:
    if ⊛ VisualQARunner
