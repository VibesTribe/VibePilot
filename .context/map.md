# VibePilot Code Map
# Generated: 2026-04-29T00:35:53Z | Commit: 4a94a00f
# Auto-generated. Run build.sh to regenerate.

## governor/cmd/cleanup/main.go
main.go [68L]
  deps: log, github.com/vibepilot/governor/internal/db, time, context, os, os/signal, syscall, fmt
  API:
    fn main()
...
content_fetcher.go [48L]
  deps: log, path/filepath, io, os, fmt, net/http, context
  API:
...
handlers_council.go [522L]
  deps: errors, log, github.com/vibepilot/governor/internal/db, github.com/vibepilot/governor/internal/gitree, context, encoding/json, fmt, sync, os, time, github.com/vibepilot/governor/internal/runtime
...
handlers_maint.go [664L]
  deps: time, github.com/vibepilot/governor/internal/db, encoding/json, os/exec, context, fmt, github.com/vibepilot/governor/internal/gitree, strings, log, github.com/vibepilot/governor/internal/runtime
...
handlers_plan.go [699L]
  deps: os, log, github.com/vibepilot/governor/internal/gitree, github.com/vibepilot/governor/internal/runtime, time, fmt, github.com/vibepilot/governor/internal/db, path/filepath, context, encoding/json
  API:
...
handlers_research.go [463L]
  deps: fmt, time, sync, github.com/vibepilot/governor/internal/db, github.com/vibepilot/governor/internal/runtime, log, context, encoding/json
...
handlers_task.go [1706L]
  deps: github.com/vibepilot/governor/internal/connectors, log, os, path/filepath, strings, time, context, encoding/json, fmt, github.com/vibepilot/governor/internal/core, github.com/vibepilot/governor/internal/gitree, github.com/vibepilot/governor/internal/runtime, github.com/vibepilot/governor/internal/db, github.com/vibepilot/governor/internal/security
...
handlers_testing.go [1226L]
  deps: time, github.com/vibepilot/governor/internal/runtime, fmt, context, path/filepath, strings, encoding/json, os, log, os/exec, github.com/vibepilot/governor/internal/gitree, bytes, github.com/vibepilot/governor/internal/db
...
main.go [699L]
  deps: github.com/vibepilot/governor/internal/vault, time, github.com/vibepilot/governor/internal/security, github.com/vibepilot/governor/internal/dag, github.com/vibepilot/governor/internal/webhooks, os/signal, github.com/vibepilot/governor/internal/tools, fmt, github.com/vibepilot/governor/internal/memory, github.com/vibepilot/governor/internal/db, path/filepath, os, github.com/vibepilot/governor/internal/runtime, context, github.com/vibepilot/governor/internal/gitree, github.com/vibepilot/governor/internal/mcp, syscall, github.com/vibepilot/governor/internal/connectors, github.com/vibepilot/governor/internal/core, github.com/vibepilot/governor/internal/pgnotify, log, encoding/json
  API:
    fn main()
...
recovery.go [443L]
  deps: github.com/vibepilot/governor/internal/core, github.com/vibepilot/governor/internal/db, fmt, encoding/json, context, log, github.com/vibepilot/governor/internal/runtime, time
  API:
...
smoke.go [252L]
  deps: time, bytes, fmt, io, os, log, net/http, encoding/json
  API:
...
startup_validate.go [503L]
  deps: os, encoding/json, strings, fmt, log, time, context, os/exec, path/filepath
  API:
...
validation.go [474L]
  deps: encoding/json, regexp, strconv, log, github.com/vibepilot/governor/internal/db, fmt, github.com/vibepilot/governor/internal/gitree, context, github.com/vibepilot/governor/internal/runtime, strings
  API:
    cl ⊛ TaskData
    cl ⊛ ValidationError
      fn ⊛ Error() → string
...
main.go [200L]
  deps: bytes, encoding/base64, io, encoding/json, fmt, log, os, golang.org/x/crypto/pbkdf2, crypto/aes, crypto/cipher, crypto/rand, net/http, crypto/sha256
  API:
    fn main()
    cl ⊛ Secret
...
courier.go [267L]
  deps: bytes, fmt, net/http, io, encoding/json, time, sync, context
...
runners.go [513L]
  deps: context, time, net/http, github.com/vibepilot/governor/internal/runtime, encoding/json, bufio, bytes, io, os/exec, strings, github.com/vibepilot/governor/internal/vault, fmt
...
state.go [299L]
  deps: fmt, sync, context, time, encoding/json
...
test_runner.go [296L]
  deps: os/exec, encoding/json, time, fmt, context, os, path/filepath, strings
...
engine.go [233L]
  deps: strings, log, context, fmt, sync, time
  exports: NewEngine
  API:
    cl ⊛ NodeOutput
    if ⊛ NodeExecutor
...
postgres.go [685L]
  deps: github.com/jackc/pgx/v5, context, fmt, encoding/binary, github.com/jackc/pgx/v5/pgxpool, time, encoding/json, strconv, strings, github.com/jackc/pgx/v5/pgtype
  exports: NewPostgres
  API:
    cl ⊛ PostgresDB
...
supabase.go [285L]
  deps: encoding/json, strings, regexp, context, net/url, bytes, net/http, fmt, io, time
...
gitree.go [976L]
  deps: context, regexp, strings, time, fmt, bytes, log, os/exec, os, path/filepath, encoding/json
  exports: New
  API:
...
managed_repo.go [295L]
  deps: os, strings, time, log, os/exec, fmt, path/filepath, context
...
worktree.go [416L]
  deps: fmt, time, log, context, strings, regexp, os, path/filepath
...
maintenance.go [346L]
  deps: github.com/vibepilot/governor/internal/db, path/filepath, fmt, log, strings, os, github.com/vibepilot/governor/internal/gitree, github.com/vibepilot/governor/pkg/types, context
  exports: New
  API:
    ty ⊛ RiskLevel
    ty ⊛ ChangeType
...
sandbox.go [165L]
  deps: log, context, time, os, fmt, os/exec, io, path/filepath
  API:
...
validation.go [248L]
  deps: path/filepath, time, encoding/json, strings, log, os, fmt
  API:
...
governor_server.go [211L]
  deps: encoding/json, fmt, strconv, os, github.com/mark3labs/mcp-go/mcp, github.com/mark3labs/mcp-go/server, log, github.com/vibepilot/governor/internal/runtime, context
...
registry.go [253L]
  deps: github.com/vibepilot/governor/internal/runtime, github.com/mark3labs/mcp-go/client/transport, sync, fmt, github.com/mark3labs/mcp-go/mcp, github.com/mark3labs/mcp-go/client, time, context, log, encoding/json
  exports: NewRegistry
  API:
    cl ⊛ ToolBinding
    cl ⊛ Registry
...
compactor.go [248L]
  deps: context, encoding/json, github.com/vibepilot/governor/internal/db, fmt, time, strings, github.com/vibepilot/governor/internal/runtime
...
listener.go [246L]
  deps: time, encoding/json, github.com/jackc/pgx/v5, fmt, github.com/vibepilot/governor/internal/runtime, log, context
  exports: NewListener
  API:
    if ⊛ SSEBroadcaster
    cl ⊛ Listener
...
config.go [1372L]
  deps: sync, encoding/json, fmt, context, log, os, path/filepath
...
connector_tracker.go [283L]
  deps: time, log, context, encoding/json, sync
...
context_builder.go [355L]
  deps: strings, encoding/json, context, path/filepath, time, sync, os, fmt
...
model_loader.go [364L]
  deps: encoding/json, github.com/vibepilot/governor/internal/db, log, time, context, path/filepath, os, fmt
...
research_action.go [561L]
  deps: sync, os, github.com/vibepilot/governor/internal/db, path/filepath, context, encoding/json, fmt, log
...
router.go [808L]
  deps: log, encoding/json, context, sort, sync/atomic, github.com/vibepilot/governor/internal/db
...
usage_tracker.go [813L]
  deps: fmt, time, encoding/json, context, log, sync
...
file_tools.go [177L]
  deps: fmt, path/filepath, strings, encoding/json, context, os
...
registry.go [94L]
  deps: net/http, github.com/vibepilot/governor/internal/gitree, time, github.com/vibepilot/governor/internal/vault, github.com/vibepilot/governor/internal/db, github.com/vibepilot/governor/internal/runtime
  exports: RegisterAll
  API:
    cl ⊛ Dependencies
    fn init()
...
sandbox_tools.go [245L]
  deps: fmt, path/filepath, os/exec, strings, encoding/json, context, bytes, os, time
...
web_tools.go [231L]
  deps: net/url, io, fmt, context, encoding/json, net/http, strings, github.com/vibepilot/governor/internal/runtime
...
vault.go [506L]
  deps: encoding/base64, crypto/rand, encoding/json, crypto/cipher, os, fmt, github.com/vibepilot/governor/internal/db, crypto/aes, context, golang.org/x/crypto/pbkdf2, io, time, crypto/sha256, sync, log
...
github.go [149L]
  deps: strings, context, log, encoding/json, github.com/vibepilot/governor/internal/db
...
server.go [873L]
  deps: encoding/json, log, fmt, crypto/hmac, io, encoding/hex, net/http, strings, time, context, crypto/sha256, github.com/vibepilot/governor/internal/runtime
